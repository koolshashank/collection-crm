 ...
 */
export function buildNocPdf({ loanNo, fullName, pan, nocDateDisp, collDateDisp, collAmtDisp, remarks }) {
  const doc = new jsPDF({ orientation: "p", unit: "mm", format: "a4", compress: true });
  const headerImg = readAsset("noc_header.jpg", "noc_Header.jpg");
  const footerImg = readAsset("noc_footer.jpg", "noc_Footer.jpg");
  ...
  cell(6, "Dear Mr. " + fullName + ",", "L", MARGIN, 10);
  setFont("italic", 9.5, [100, 100, 100]);
  cell(6, "Greetings!", "L", MARGIN, 9.5);
  y += 4; // Ln(4)

  /* ── BODY PARAGRAPHS (identical text/order to PHP) ── */
  const body = (txt) => {
  ...
  body(
    "This is to certify that you have availed a loan facility bearing Loan Account Number " +
      loanNo +
      " from BlinkR Loan, a brand of RBI Registered NBFC: Dev-Aashish Capitals Private Limited."
  );
  body(
    "We hereby confirm that the aforesaid loan account has been fully closed on " +
      collDateDisp +
      " upon receipt of the entire repayment amount due and payable against the said loan account as per Loan Agreement."
  );
  body(
  ...
  body("The Company has no objection to the closure of the said loan account.");
  body("This No Objection Certificate is issued at your request for your record and future reference.");
  ...
  ensureSpace(40);
  setFont("normal", 9.5, [30, 30, 30]);
  cell(6, "Regards,", "L", MARGIN, 9.5);
  setFont("bold", 10, NAVY);
  cell(6, "Team BlinkR Loan", "L", MARGIN, 10);
  ...
  cell(5, "DEV-AASHISH CAPITALS PRIVATE LIMITED (NBFC)", "L", MARGIN, 9);
  y += 8; // Ln(8)
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    if (headerImg) {
      try {
        doc.addImage(headerImg, "JPEG", 0, 0, PG_W, HDR_H);
      } catch {
    ...
    if (footerImg) {
      try {
        doc.addImage(footerImg, "JPEG", 0, yImg, PG_W, FTR_H);
      } catch {
        drawFooterFallback(doc, yImg);
      }
    } else {
      drawFooterFallback(doc, yImg);
    }
...
function drawFooterFallback(doc, yImg) {
  doc.setFillColor(13, 52, 100);
  doc.rect(0, yImg, PG_W, FTR_H, "F");
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(255, 255, 255);
  // Branding line — override per company via NOC_FOOTER_TEXT in .env.local
  const footerText =
    process.env.NOC_FOOTER_TEXT || "RBI Registered NBFC | +91-8595333222 | Info@blinkrloan.com";
  doc.text(footerText, PG_W / 2, yImg + 15, { align: "center" });
}

/** Absolute paths of the inline-email images (or null when missing). */
export function nocImagePaths() {
  return {
    header: assetPath("noc_header.jpg", "noc_Header.jpg"),
    footer: assetPath("noc_footer.jpg", "noc_Footer.jpg"),

...
/* Deepija's "Location 1 / Dev" instance (blinkrloandev.deepijatel.in,
   192.168.0.155) belongs to a different client and must NEVER be used by
   this project — only Location 2 / Production (192.168.0.154) is wired
   up here. Do not re-add a second location without explicit sign-off. */
...
/**
 * Resolves a CRM email to its ConVox location config. This project only
 * ever talks to Location 2 (Production, 192.168.0.154) — there is no
 * per-user override, so a misconfigured env var can't accidentally route
 * a call to the other client's Dev server.
 */
export function convoxGetLocationForEmail(_email) {
  return CONVOX_LOCATIONS[2];
}

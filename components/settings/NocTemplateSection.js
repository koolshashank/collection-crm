...
const PLACEHOLDER_HINTS = [
  ...
  ["{pan}", "PAN"],
  ["{nocDateDisp}", "NOC date"],
  ["{collDateDisp}", "Closure/collection date"],
  ["{collAmtDisp}", "Collected amount"],
...
/**
 * NOC Template — the certificate's title, salutation, body paragraphs and
 * closing lines, editable from Settings instead of hardcoded in
 * lib/noc/pdf.js. Both NOC generate and NOC email use the same builder, so
 * one edit here updates both. Preview renders a real sample PDF (fake loan
 * data) from the current unsaved draft before you commit to Save Changes.
 */
export default function NocTemplateSection({ config, defaults, onChange }) {
  ...
  function restoreDefaults() {
    onChange({ ...defaults });
    toast.success("Restored the default NOC template — remember to Save Changes.");
  }

  async function openPreview() {
    setPreviewLoading(true);
    try {
      const res = await fetch("/api/noc/preview", {
        method: "POST",
    ...
    <div className="card mb-5 overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-line bg-surface px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display text-sm font-bold text-gray-800">NOC Template</div>
            <div className="mt-0.5 text-xs text-gray-500">Edit the certificate's wording — no code changes needed next time</div>
          </div>
        </div>
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start gap-2.5 rounded-xl border border-accent-light bg-accent-light/40 px-3.5 py-3 text-xs leading-relaxed text-gray-600">
          ...
          <div>
            <p>
              Use these tokens anywhere below — they're filled in with the real loan's details when a NOC is generated
              or emailed:
            </p>
        ...
        <Field
          ...
          hint="Shown as the opening line, e.g. “Dear Mr. {fullName},”"
        />
      ...
      <Modal
        ...
        size="full"
        title="NOC Preview — sample loan data"
        footer={
      ...
      >
        {previewUrl && (
          <iframe src={previewUrl} title="NOC preview" className="h-[75vh] w-full rounded-lg border border-line" />
        )}
      </Modal>
    </div>
  );

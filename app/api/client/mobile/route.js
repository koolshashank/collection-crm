...
/**
 * GET /api/client/mobile?pan=…
 * Port of client_info.php step 2: GET {apiBase}/getMobileNumber/{pan}.
...
export async function GET(request) {
  try {
    ...
    const res = await apiGet("get_mobile", { suffix: encodeURIComponent(pan), timeoutMs: 12000 });
    if (res.error) {
    ...
    return NextResponse.json({ success: false, message: "Failed to load mobile data." }, { status: 500 });
  }
}
